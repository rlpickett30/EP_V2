...
class PlatformRegistryRegistryManager:
    ...
    def __init__(self, config=None):
        ...
        self.registry_config = self.config.get(
            "platform_registry",
            {}
        ).get(
            "registry",
        ...
        self.allowed_node_roles = self.config.get(
            "platform_registry",
            {}
        ).get(
            "allowed_node_roles",
        ...
        self.allowed_gui_roles = self.config.get(
            "platform_registry",
            {}
        ).get(
            "allowed_gui_roles",
    ...
    def register_node(self, payload):
        ...
        if self.require_node_id and not node_id:
            return self._fail(
                result,
                "Node registration failed. Missing node_id.",
                payload
            )
        ...
        if existing_record is None:
            ...
            record["node_name"] = payload.get(
                "node_name",
                record.get("node_name", node_id)
            )
    ...
    def register_gui(self, payload):
        ...
        gui_id = payload.get("gui_id")

        if self.require_gui_id and not gui_id:
            return self._fail(
                result,
                "GUI registration failed. Missing gui_id.",
                payload
            )
        ...
        gui_role = payload.get("gui_role", "primary_gui")

        if not self._role_allowed(gui_role, self.allowed_gui_roles):
            return self._fail(
                result,
                f"GUI registration failed. Invalid GUI role: {gui_role}",
                payload
            )
        ...
        if existing_record is None:
            record = {
                "gui_id": gui_id,
                "gui_name": payload.get("gui_name", gui_id),
                "gui_role": gui_role,
            ...
            record["gui_name"] = payload.get(
                "gui_name",
                record.get("gui_name", gui_id)
            )
