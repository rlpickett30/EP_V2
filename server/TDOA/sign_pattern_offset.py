...
#
# Purpose:
#   Detect offset candidates using sign-transition patterns.
#
# Expected config source:
#   TDOA_config.json
#
# Expected config section:
#   config["sign_pattern_offset"]
#
# Does:
...
def detect_sign_pattern_offsets(
    signal,
    config,
    sample_rate_hz,
    debug=False
    ...
    This is the offset-side partner to sign_pattern_onset.py.
    ...
    config:
        Full TDOA config dictionary or the sign_pattern_offset
    ...
    try:
        ...
        max_forwardtrack_samples = detector_config.get(
            "max_forwardtrack_samples",
            7
        )
        ...
        if max_forwardtrack_samples < 0:
            raise ValueError(
                "max_forwardtrack_samples cannot be negative."
            )
        ...
        while i < len(signal) - run_length * 2:
            ...
            if valid_pattern:
                ...
                forwardtrack_count = 0

                while (
                    candidate_index < len(signs) - 1
                    and signs[candidate_index + 1] == target_sign
                    and forwardtrack_count < max_forwardtrack_samples
                ):
                    candidate_index += 1
                    forwardtrack_count += 1
